...
//learning params
	...
	std::string weight_save_path = "ac_weights.json";
...
//weights
	...
	/*
	auto release_deadtime = [&](const ros::TimerEvent &event)
	{
	...
	// */
	//*
	//auto restart_cb = [&](const ros::TimerEvent &event)
	...
	auto reset_cb = [&](json_t *msg)
	{
		...
		is_first = true;
		std::printf("restarted actor critic\n");
		//dead_timer = n.createTimer(ros::Duration(0.1), release_deadtime, true);
		//is_stop_status_by_edge = false;
	};
	// */
	...
	{
		//input_vec = const_cast<const std::vector<float>&>(msg->data);
		/*
		...
		if(latest_input_vec.size() > dim_input)
		{
			...
			//delta_w_i.resize(dim_input);
		}
		// */
		/*
		for(int i=0;i<dim_input;++i)
			input_vec[i] = msg->data[i];
		// */
	};

	//auto reward_cb = [&](const std_msgs::Float32::ConstPtr& msg)
	auto reward_cb = [&](json_t* msg)
	{
		...
		json_decref(data);
		//*
		//if(std::abs(reward) < learning_restart_threshold && !waiting_for_restart)// && is_stop_status_by_edge)
		if(std::abs(reward) > learning_stop_threshold && !(stop_in_n_loop) && !waiting_for_restart)// && is_stop_status_by_edge)
		{
			//is_stop_status_by_edge = false;
			//waiting_for_restart = true;
			stop_in_n_loop = 3;
			//ROS_INFO("stopping actor critic");
			//timer = n.createTimer(ros::Duration(1.), restart_cb, true);
		}
		// */
	};
	...
	std::string input_topic = "/pc_activity";
